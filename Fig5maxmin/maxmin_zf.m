function [r, TotalTime, SolveTime] = maxmin_zf(params)
...
%%%%%%%%%%%%%%%%%%ZFBF%%%%%%%%%%%%%%%%%%
H_pinv=H*inv(H'*H);  %pseudo-inverse
...
subject to
     for l=1:L    %%%Transmit Power Constraints
         %%        
         if l==1
             norm(V_zf(1:params.N_set(1),:)*diag(Q),'fro')<=sqrt(params.P_set(l));
         else
             norm(V_zf(sum(params.N_set(1:l-1))+1:sum(params.N_set(1:l)),:)*diag(Q),'fro')<=sqrt(params.P_set(l));
         end
     end

     for k=1:K    %%%%%%%%%QoS constraints
      %% 
           norm([params.H(:,k)'*V_zf*diag(Q), params.delta_set(k)])<=sqrt(1+1/params.r_set(k))*norm(params.H(:,k)'*V_zf(:,k))*Q(k);   
     end
