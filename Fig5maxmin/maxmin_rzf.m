function [r,TotalTime,SolveTime] = maxmin_rzf(params)
...
%%%%%%%%%%%%%%%%%%Regularized ZFBF%%%%%%%%%%%%%%%%%%
H_pinv=H*inv(eye(K)+sum(params.P_set)/K*H'*H);  %pseudo-inverse
...
subject to
     for l=1:L    %%%Transmit Power Constraints
         %%
         if l==1
             norm(V_rzf(1:params.N_set(1),:)*diag(Q),'fro')<=sqrt(params.P_set(l));
         else
             norm(V_rzf(sum(params.N_set(1:l-1))+1:sum(params.N_set(1:l)),:)*diag(Q),'fro')<=sqrt(params.P_set(l));
         end
     end

     for k=1:K    %%%%%%%%%QoS constraints
      %%   
           norm([params.H(:,k)'*V_rzf*diag(Q), params.delta_set(k)])<=sqrt(1+1/params.r_set(k))*norm(params.H(:,k)'*V_rzf(:,k))*Q(k);   
     end
%cvx_end
