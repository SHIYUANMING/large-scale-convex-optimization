...
for lp=1:LC
    ...
    for lq=1:length(Q)
        ...
        TotalTime_record=0;  %record time at each SINR target
        SolveTime_record=0;
...
%%
 if feasible_cvx==1 
     ...
     for A=0:L-2
         ...
         %% Call CVX
         tic;
         [Wsolution,feasible, solving_time] = powermin_cvx(params);  %power minimization given the active RRH set
         TotalTime_record=TotalTime_record+toc;
         SolveTime_record=SolveTime_record+solving_time;
...
%%
    if norm(TotalPower)<10^100
     ...
     TransTime_temp=TransTime_temp+(TotalTime-SolveTime);
     SolveTime_temp=SolveTime_temp+SolveTime;
     TotalTime_temp=TotalTime_temp+TotalTime;
