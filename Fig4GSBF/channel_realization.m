...
for k=1:K
    for l=1:L
        if l==1
            ...
            H(1:N_set(1),k)=D(l,k)*(normrnd(0,1/sqrt(2),N_set(1),1));  %%%nosie normalized to 1
        else
           ...
           H(sum(N_set(1:(l-1)))+1:sum(N_set(1:l)),k)=D(l,k)*(normrnd(0,1/sqrt(2),N_set(l),1));  %%%nosie normalized to 1
        end
    end
end
